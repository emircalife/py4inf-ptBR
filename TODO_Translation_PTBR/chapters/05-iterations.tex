...
Um padrão comum em instruções de atribuição é uma declaração de atribuição
que atualiza uma variável --
...
\afterverb
%
Isso significa ``pega o valor atual de {\tt x}, adiciona 1, e então
atualiza {\tt x} com o novo valor.''

Se você tentar atualizar uma variável que não exista, você vai obter um
erro, porque Python avalia o lado direito antes de atribuir 
um valor para {\tt x}:
...
\afterverb
%
Antes que você pode atualizar uma variável , você tem que {\ bf initializar }
que, geralmente com uma simples assinatura:

\index{initialization (antes de atualizar)}
...
\afterverb
%
Atualizando uma variável adicionando 1 é chamado um {\bf incremento};
subtraindo 1 é chamado um {\bf decremento}.
